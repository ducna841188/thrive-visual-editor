...
function tve_get_shared_styles( $post_content = '', $for_media = '', $editor_ajax_check = true ) {
	...
	/**
	 * Don't include default styles on LPs by default - unless the "do not strip head css" option for the LP is checked
	 */
	$output_default_styles = ! tcb_post()->is_landing_page() || ! tcb_landing_page( get_the_ID() )->should_strip_head_css();
...
if ( ! function_exists( 'tve_frontend_enqueue_scripts' ) ) {
	...
	function tve_frontend_enqueue_scripts() {
		...
		if ( $frontend_options['is_single'] ) {
			$frontend_options['post_id']    = get_the_ID();
 ...
 * @return array
 */
function tve_get_default_styles() {
	/**
	 ...
	 * @return array
	 */
	return apply_filters( 'tcb_default_styles', tcb_default_style_provider()->get_styles() );
}
