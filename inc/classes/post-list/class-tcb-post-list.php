...
class TCB_Post_List {
	...
	public function init_query( $attr_query ) {
		...
		if ( ! empty( $this->query['s'] ) && is_search() ) {
			global $wp_query;
			$this->query['s'] = $wp_query->query['s'];
		}

		/*When the Post List is on a date archive page, the Featured List should also be updated*/
		if ( ! empty( $this->query['year'] ) && ! empty( $this->query['monthnum'] ) && is_date() ) {
			global $wp_query;
			$this->query['year']     = (int) $wp_query->query['year'];
			$this->query['monthnum'] = (int) $wp_query->query['monthnum'];
		}
	}
