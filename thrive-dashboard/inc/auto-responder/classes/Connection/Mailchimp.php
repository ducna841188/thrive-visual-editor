...
class Thrive_Dash_List_Connection_Mailchimp extends Thrive_Dash_List_Connection_Abstract {
	...
	public function addSubscriber( $list_identifier, $arguments ) {
		...
		try {
			...
			if ( ! empty( $arguments['tve_mapping'] ) ) {
				$this->updateCustomFields( $list_identifier, $arguments, $data );
			}
		} catch ( Thrive_Dash_Api_Mailchimp_Exception $e ) {
	...
	public function updateCustomFields( $list_identifier, $arguments, $data ) {
		...
		if ( ! $list_identifier || empty( $arguments ) || empty( $data ) ) {
			return false;
		}
		...
		try {
			...
			$user_hash = md5( $data['email_address'] );
			...
			// Update contact with custom mapped fields
			$api->request( 'lists/' . $list_identifier . '/members/' . $user_hash, $data, 'PUT' );

			$return = true;
		} catch ( Thrive_Dash_Api_Mailchimp_Exception $e ) {
			// Log api errors
			$this->api_log_error( $list_identifier, $data, __METHOD__ . ': ' . $e->getMessage() );
			$return = $e->getMessage();
		} catch ( Exception $e ) {
			// Log api errors
			$this->api_log_error( $list_identifier, $data, __METHOD__ . ': ' . $e->getMessage() );
